...
test.describe("circle cursor", () => {
  test("follows the pointer and expands over interactive elements", async ({
    ...
    await expect
      .poll(() =>
        ring.evaluate((element) => Number(getComputedStyle(element).opacity)),
      )
      .toBeGreaterThan(0.5);
    ...
    await expect
      .poll(() =>
        ring.evaluate((element) => {
          ...
          return Math.hypot(
            rect.left + rect.width / 2 - 320,
            rect.top + rect.height / 2 - 280,
          );
        }),
      )
      .toBeLessThan(3);
    ...
    await expect
      .poll(() =>
        ring.evaluate((element) => element.getBoundingClientRect().width),
      )
      .toBeGreaterThan(70);
    ...
    await expect
      .poll(async () =>
        Number(
          await dot.evaluate((element) => getComputedStyle(element).opacity),
        ),
      )
      .toBeLessThan(0.05);
    ...
    await expect
      .poll(() =>
        ring.evaluate((element) => {
          const rect = element.getBoundingClientRect();
          return Math.hypot(
            rect.left + rect.width / 2 - 700,
            rect.top + rect.height / 2 - 300,
          );
        }),
      )
      .toBeLessThan(3);
  ...
  test("keeps the cursor on the pointer while the menu is open", async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name === "Mobile Chrome",
  ...
  test("forces the custom cursor when reduced motion is requested", async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name === "Mobile Chrome",
