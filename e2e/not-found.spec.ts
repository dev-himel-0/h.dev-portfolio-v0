...
test.describe("not found", () => {
  ...
  test("draws fuzzy canvases for the status and the message", async ({ page }) => {
    await page.goto(MISSING_PATH)
    ...
    await expect
      .poll(() => status.evaluate((element) => element.getBoundingClientRect().width))
      .toBeGreaterThan(100)
    await expect
      .poll(() => status.evaluate((element) => element.getBoundingClientRect().height))
      .toBeGreaterThan(40)

    const message = canvases.nth(1)
    await expect
      .poll(() => message.evaluate((element) => element.getBoundingClientRect().width))
      .toBeGreaterThan(100)
    await expect
      .poll(() => message.evaluate((element) => element.getBoundingClientRect().height))
      .toBeGreaterThan(8)
  })
