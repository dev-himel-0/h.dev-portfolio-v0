...
test.describe("responsive layout", () => {
    ...
    test(`stays inside the viewport at ${viewport.name}`, async ({ page }) => {
      ...
      const layout = await page.evaluate(() => {
        const visibleOutliers = [...document.querySelectorAll("body *")]
          ...
          .filter(
            ({ left, right, width }) => width > 0 && (left < -1 || right > window.innerWidth + 1),
          )
  ...
  test("wraps narrow footer socials without clipping", async ({ page }) => {
    ...
    const items = await page.locator("[data-footer-socials] > li").evaluateAll((elements) =>
    ...
    expect(items.every(({ left, right }) => left >= 0 && right <= viewportWidth)).toBe(true)
  })

  test("keeps service titles clear of their icons at the smallest width", async ({ page }) => {
    await page.setViewportSize({ width: 280, height: 640 })
    await page.goto("/")

    const rows = await page.locator("[data-service-row]").evaluateAll((elements) =>
      elements.map((element) => {
        ...
        return title.scrollWidth <= title.clientWidth && titleRect.right <= iconRect.left
      }),
    )
  ...
  for (const viewport of [
    ...
    test(`fits the open menu at ${viewport.name}`, async ({ page }) => {
      ...
      const menu = await page.locator("#staggered-menu-panel").evaluate((element) => ({
