    ...
    ({ selector }) =>
      new Promise<number[]>((resolve) => {
        ...
        const frame = (n: number) => {
          ...
          flags.push(!!curtain && getComputedStyle(curtain).display !== "none" ? 1 : 0)
...
test.describe("page transition", () => {
  ...
  test("keeps the wipe curtain mounted but hidden after arrival", async ({ page }) => {
    await page.goto(MISSING_PATH)
    await expectWipeResting(page)
  ...
  test("back navigation returns through the transition to the 404 page", async ({ page }) => {
    await page.emulateMedia({ reducedMotion: "no-preference" })
    await page.goto(MISSING_PATH)
    await expectWipeResting(page)
    await expect(page.getByRole("link", { name: notFound.actions[0].label })).toBeVisible()
  ...
  test("plays an entrance wipe on a directly-loaded 404 page", async ({ page }) => {
    await page.emulateMedia({ reducedMotion: "no-preference" })
    await page.goto(MISSING_PATH)
  ...
  test("home hard loads keep the preloader without an entrance wipe", async ({ page }) => {
    await page.emulateMedia({ reducedMotion: "no-preference" })
  ...
  test("stays within the viewport on mobile after navigating", async ({ page }) => {
