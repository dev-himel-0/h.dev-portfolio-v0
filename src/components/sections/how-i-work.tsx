  ...
  useGSAP(
    () => {
      ...
      steps.forEach((step, index) => {
        ...
        const copy = gsap.utils.toArray<HTMLElement>(step.querySelectorAll("[data-process-copy]"))
        const numberStrip = step.querySelector<HTMLElement>("[data-process-number-strip]")
        const verticalFills = gsap.utils.toArray<HTMLElement>(
          step.querySelectorAll("[data-process-vertical-fill], [data-process-mobile-fill]"),
        )
        const horizontalFill = step.querySelector<HTMLElement>("[data-process-horizontal-fill]")
    ...
    >
      ...
      <div className="mx-auto w-full max-w-[75rem] px-[1.875rem]">
        <header className="mb-[3.75rem] flex w-full flex-col items-end gap-7 text-right sm:mb-[5rem] lg:mb-[6.25rem]">
          <span data-process-label className="sr-only">
        ...
        <ol ref={flowRef} data-process-flow className="relative max-[809px]:pr-0 max-[809px]:pl-5">
          <span
            aria-hidden="true"
    ...
    >
      ...
      >
        ...
        >
          ...
          <span aria-hidden="true" className="absolute top-0 left-0 flex">
            <span>0</span>
            <span
        ...
        <div data-process-copy className="mt-9 flex items-center gap-4">
          <Image
            data-process-icon
            src={icon.src}
          ...
          />
          <h3 className="font-heading text-[clamp(1.5rem,2.5vw,2rem)] leading-none font-semibold tracking-[-0.03em]">
            {step.title}
          </h3>
        </div>
        <p
          data-process-copy
          className="mt-3 max-w-[30rem] font-sans text-sm leading-[1.65] tracking-[-0.005em] text-black/60"
        >
          {step.description}
        </p>
      </div>
