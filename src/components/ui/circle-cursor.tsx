    ...
    const draw = (time: number) => {
      ...
      activeSparks.forEach((spark) => {
        const progress = Math.min(
          Math.max((time - spark.startedAt) / SPARK_DURATION, 0),
          1
        );
        ...
        context.beginPath();
        context.moveTo(
          spark.x + directionX * distance,
          spark.y + directionY * distance
        );
        context.lineTo(
      ...
      frameRef.current =
        activeSparks.length > 0 ? window.requestAnimationFrame(draw) : null;
    };
