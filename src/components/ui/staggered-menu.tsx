...
export function StaggeredMenu({
  ...
  const resetClosed = useCallback(() => {
    ...
    gsap.set(panel.querySelectorAll("[data-menu-social-link]"), {
      y: 25,
      opacity: 0,
    });
  ...
  const playOpen = useCallback(() => {
    ...
    const prelayers = Array.from(
      root.querySelectorAll<HTMLElement>("[data-menu-prelayer]"),
    );
    const labels = Array.from(panel.querySelectorAll("[data-menu-item-label]"));
    const anchors = Array.from(
      panel.querySelectorAll<HTMLElement>(".smg-item"),
    );
    ...
    tl.to(
      ...
      {
        opacity: 1,
        duration: MENU_TIMING.panelDuration,
        ease: YUNOX_MENU_EASE,
      },
    ...
    const cascadeEnd =
      itemsStart +
      MENU_TIMING.itemDuration +
      (labels.length - 1) * MENU_TIMING.itemStagger;
  ...
  useLayoutEffect(() => {
    ...
    const wave = (row: HTMLElement, on: boolean) => {
      ...
      const clone = Array.from(
        row.querySelectorAll<HTMLElement>(".smg-char-b"),
      );
    ...
    return () => {
      ...
      root
        .querySelectorAll<HTMLElement>(".smg-char")
        .forEach((el) => gsap.killTweensOf(el));
      root
        .querySelectorAll<HTMLElement>(".smg-item")
        .forEach((el) => gsap.killTweensOf(el));
    };
  }, []);
      ...
      >
        <div className="smg-inner">
          ...
          <footer className="smg-footer">
            <div
              data-menu-footer-lead
              className="smg-footer-lead"
            >
              <a
                className="smg-email"
            ...
            <div className="smg-socials">
              <span
                data-menu-socials-title
                className="smg-socials-label"
              >
                Socials
              </span>
              <ul
                className="smg-socials-list"
                role="list"
              >
                {socials.map((s, i) => {
            ...
            <p
              data-menu-footer-note
              className="smg-footer-note"
            >
